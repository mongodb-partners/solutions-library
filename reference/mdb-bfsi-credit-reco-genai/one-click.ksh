...
# Variables
INFRA_STACK_NAME="MAAP-Fireworks-Stack-Infrastructure"
EC2_STACK_NAME="MAAP-Fireworks-Stack-Compute"

INFRA_TEMPLATE_FILE="deploy-infra.yaml"
# INFRA_TEMPLATE_FILE="deploy-infra-internal.yaml"
...
ServerInstanceProfile="MAAPFireworksServerInstanceProfile"
IMDSv2LaunchTemplateName="IMDSv2"
VolumeSize=100 #Disk Storage Space

GIT_REPO_URL="https://github.com/ashwin-gangadhar-mdb/mdb-bfsi-credit-reco-genai.git" #GitHub Repository URL

TAG_NAME="MAAP-Fireworks-One-Click"

MongoDBClusterName="MongoDBFireworksV1"
MongoDBUserName="fireworksuser"
MongoDBPassword="fireworksuser123"
...
{
  # Parameters
  KEY_NAME="MAAPFireworksKeyV1"  # You can change the key pair name as desired.
...
deploy_ec2()
{
  ...
  echo "MongoDB Group ID:" $GROUP_ID
  echo "MongoDB Cluster Name:" $MongoDBClusterName
  echo "MongoDB User Name:" $MongoDBUserName
  echo "MongoDB Password:" $MongoDBPassword
  echo "API Public Key:" $API_PUBLIC_KEY
  echo "API Private Key:" $API_PRIVATE_KEY
  ...
  log_message "$(aws cloudformation create-stack \
    ...
    --parameters \
      ...
      ParameterKey=APIPUBLICKEY,ParameterValue="$API_PUBLIC_KEY" \
      ParameterKey=APIPRIVATEKEY,ParameterValue="$API_PRIVATE_KEY" \
      ...
      ParameterKey=FireworksAPIKey,ParameterValue="$FIREWORKS_API_KEY" \
      ParameterKey=FireworksModelID,ParameterValue="$FIREWORKS_MODEL_ID" \
      ParameterKey=IMDSv2LaunchTemplateName,ParameterValue="$IMDSv2LaunchTemplateName" \
      ParameterKey=GROUPID,ParameterValue="$GROUP_ID")"
  ...
  FULL_URL="http://$URL:3000"
  BACKEND_URL="http://$URL:5001"

  log_message "The application will be available at $FULL_URL once all the deployment is complete."
  log_message "The backend service will be available at $BACKEND_URL."
